...
inline int dj(){
  ...
  while(!pq.empty()){
    ...
    for(int i=head[x];i;i=Next[i]){
      ...
      if(edge[i]==0){
        if(sum<k){
          if(dist[y][sum+1]>tep.val){
            dist[y][sum+1]=tep.val;
            pq.push({sum+1,tep.val,y});}
          else{continue;}
        }
      }
      else{
        if(dist[y][sum]>max(tep.val,edge[i])){
          dist[y][sum]=max(tep.val,edge[i]);
          pq.push({sum,max(tep.val,edge[i]),y});
        }
      }
    }
  }
